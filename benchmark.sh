#!/usr/bin/env bash
#
# Description: Bench Script (Teddysun Logic)
# Reskinned By: Kuli-Korporat
#
...
_exit() {
    ...
    # clean up logic persis teddysun
...
# --- SPEEDTEST LOGIC (JIPLAK TOTAL TEDDYSUN) ---
# Ini kuncinya: Dia save ke log dulu baru dibaca grep/awk
speed_test() {
    local nodeName="$2"
    if [ -z "$1" ];then
...
# Daftar Server (Boleh diganti ID-nya, tapi logic pemanggilannya tetap)
speed() {
    speed_test '' 'Speedtest.net'
    speed_test '11362' 'Jakarta, ID'      
...
# --- INSTALL LOGIC (JIPLAK TOTAL TEDDYSUN) ---
# Menggunakan URL dan folder struktur yang sama persis
install_speedtest() {
    if [ ! -e "./speedtest-cli/speedtest" ]; then
        ...
        # URL Download Original Teddysun (Versi 1.2.0 Stabil)
...
print_intro() {
    # HANYA INI YANG SAYA UBAH (BRANDING)
    echo -e "$(_blue " _   __      _ _      ")"
    echo -e "$(_blue "| | / /     | (_)     ")"
    echo -e "$(_blue "| |/ / _   _| |_      ")"
    echo -e "$(_blue "|    \| | | | | |     ")"
    echo -e "$(_blue "| |\  \ |_| | | |     ")"
    echo -e "$(_blue "\_| \_/\__,_|_|_|     ")"
    echo "------------------ Bench Script By Kuli-Korporat -----------------"
    echo " Version            : $(_green v2025-11-24)"
    echo " Usage              : $(_red "wget -qO- [url] | bash")"
}
